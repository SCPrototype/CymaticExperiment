Shader "Custom/Electric wire" {
	Properties{
		_MainTex("Albedo (RGB)", 2D) = "white" {}
		_NormalTex("Normal", 2D) = "white" {}
		_OcclusionTex("Occlusion", 2D) = "white" {}
		//_Glossiness("Smoothness", 2D)) = "white" {}
		_MetalicGlossiness("Metallic-Smoothness", 2D) = "white" {}
		...
		SubShader{
			...
			Pass
			{
				...
				vertexToFragment myVertexShader(vertexInput v) {
					...
					//o.uv = v.vertex;
					o.uv = mul(UNITY_MATRIX_M, v.vertex);
					return o;
				}
				...
				fixed4 myFragmentShader(vertexToFragment i) : SV_Target{
					...
					float4 direction = normalize(_GlowDirection);

					float interval = _GlowSpeed * _GlowInterval;
					float timeScale = (((i.uv.x * -direction.x) + (i.uv.y * -direction.y) + (i.uv.z * -direction.z) + _Time.y * _GlowSpeed) % interval) / interval;
					float t = saturate(1 - timeScale * (1 / _GlowLength));

					float4 color = _GlowColor * t + _GlowTraceColor * (1 - t);


					float4 output = tex2D(_GlowTex, i.uv) * color;
					return output;
				}
				ENDCG
			}
